...
int main() {
  ...
  */
  ...
  // O(1) to access the top / max element
    const auto *score =  &pq.top().first;
    const string *name = &pq.top().second;
  
    printf("%s has %d $\n", (*name).c_str(), *score); // Corregido para usar c_str()
    ...
    printf("%s has %d $\n", name->c_str(), *score); // Corregido para usar c_str() nuevamente

    pq.pop();
    printf("%s has %d $\n", (*name).c_str(), *score); // Y aquí también
